...
#FILE_DROP_LOCATION = '/home/captcha22/Documents/Captcha22/Unsorted/' #Change this folder location for your captcha server
#CAPTCHA_SERVER_LOCATION = '/home/captcha22/Documents/Captcha22/' #Change this folder location for your captcha server
...
class data_source:
    def __init__(self, args):
        self.args = args
        self.FILE_DROP_LOCATION = args.filedrop
        self.CAPTCHA_SERVER_LOCATION = args.serverlocation
        self.MODELS = args.modelfolder
        self.WORK = args.workfolder

        self.max_tokens = args.maxtokens
        self.users = []
        self.user_file = args.userfile
    ...
    def change_model_status(self, captcha, status):
        file_location = self.WORK + captcha['username'].replace(" ", "")  + "/" + captcha['title'].replace(" ","") + "/" + str(captcha['modelNumber'])
    ...
    def get_captcha_solution(self, captcha, image):
        ...
        url = "http://127.0.0.1:" + str(update['modelPorts'] + 1) + "/v1/models/" + str(update['modelName']) + ":predict"
    ...
    def get_update(self, folder):
        ...
        try:
            ...
            update['busyTraining'] = ast.literal_eval(lines[1].replace("\n",""))
            update['hasModel'] = ast.literal_eval(lines[2].replace("\n",""))
            update['modelActive'] = ast.literal_eval(lines[3].replace("\n",""))
            update['modelPorts'] = ast.literal_eval(lines[4].replace("\n",""))
            update['currentTrainingLevel'] = ast.literal_eval(lines[5].replace("\n",""))
            update['image_width'] = ast.literal_eval(lines[6].replace("\n",""))
            update['image_height'] = ast.literal_eval(lines[7].replace("\n",""))
            update['last_step'] = ast.literal_eval(lines[8].replace("\n",""))
            update['loss'] = ast.literal_eval(lines[9].replace("\n",""))
            update['perplexity'] = ast.literal_eval(lines[10].replace("\n",""))
            update['checkpoint'] = ast.literal_eval(lines[11].replace("\n",""))
    ...
    def get_results(self, captcha):
        file_location = self.WORK + captcha['username'].replace(" ", "")  + "/" + captcha['title'].replace(" ","") + "/" + str(captcha['modelNumber']) + "/test.txt"
...
class GetResultsAPI(Resource):
    ...
    def get(self, dataToken):
        ...
        captcha = [captcha for captcha in self.data_source.captchas if captcha['dataToken'] == dataToken]
        if len(captcha) == 0:
            abort(404)#return make_response(jsonify({'message': 'Unauthorized access'}), 403)
...
class CaptchaListAPI(Resource):
    ...
    def post(self):
        ...
        count = self.build_filename(posted_data['title'], get_username(self.data_source))#auth.username())
        filename = FILE_DROP_LOCATION + get_username(self.data_source).replace(" ", "") + "_" + posted_data['title'].replace(" ", "") + "_" + str(count) + ".zip"
        tempFile = open(filename, 'wb')
        tempFile.write(posted_file)
        tempFile.close()
        uid = str(uuid.uuid1())
        id_str = self.data_source.captchas[-1]['id'] + 1 if len(self.data_source.captchas) > 0 else 1
        captcha = {
            'id': id_str,
            'title': posted_data['title'],
            'username' : get_username(self.data_source),#auth.username(),
            'modelNumber': count,
            'dataToken' : uid,
            'busyTraining': False,
            'hasModel' : False,
...
class ExportAPI(Resource):
    ...
    def get(self, dataToken):
        ...
        file_location = self.data_source.MODELS + captcha[0]['username'].replace(" ", "")  + "/" + captcha[0]['title'].replace(" ","") + "/" + str(captcha[0]['modelNumber']) + "/exported-model"
...
class ToggleModelAPI(Resource):
    ...
    def post(self):
        ...
        captcha = [captcha for captcha in self.data_source.captchas if captcha['dataToken'] == args['dataToken']]
        if len(captcha) == 0:
            abort(404)
        if not self.validate_user(captcha[0]):
            abort(403)
...
class SolveCaptchaAPI(Resource):
    ...
    def post(self):
        ...
        answer = self.data_source.get_captcha_solution(captcha[0], str(args['image']))
...
class CaptchaAPI(Resource):
    ...
    def get(self, id):
        ...
        self.data_source.update_all_captchas()
        captcha = [captcha for captcha in self.data_source.captchas if captcha['id'] == id]
        if len(captcha) == 0:
            abort(404)
        if not self.validate_user(captcha[0]):
            abort(403)
    ...
    def delete(self, id):
        if (not self.auth_source.get_and_verify()):
            return make_response(jsonify({'message': 'Unauthorized access'}), 403)
        captcha = [captcha for captcha in self.data_source.captcas if captcha['id'] == id]
        if len(captcha) == 0:
            abort(404)
        if not self.validate_user(captcha[0]):
            abort(403)
    ...
    def get(self):
        ...
        for user in self.data_source.users:
            if username == user.username:
                ...
                lst = [random.choice(string.ascii_letters + string.digits) for n in range(128)]
                str_ = "".join(lst)
...
class ApiServer:
    def __init__(self,args):
        ...
        self.host = args.host
        self.port = args.port
        self.debug = args.isDebug

        self.source = data_source(args)

        self.systemAuth = AuthSystem(self.source)

        self.api.add_resource(CaptchaListAPI, '/captcha22/api/v1.0/captchas', endpoint='captchas', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(CaptchaAPI, '/captcha22/api/v1.0/captchas/<int:id>', endpoint='captcha', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(ExportAPI, '/captcha22/api/v1.0/export_model/<string:dataToken>', endpoint='export_model', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(GetProgressAPI, '/captcha22/api/v1.0/training_update/<string:dataToken>', endpoint='training_update', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(GetResultsAPI, '/captcha22/api/v1.0/results/<string:dataToken>', endpoint='results', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(ToggleModelAPI, '/captcha22/api/v1.0/activate_model', endpoint='activate_model', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(SolveCaptchaAPI, '/captcha22/api/v1.0/solve_captcha', endpoint='solve_captcha', resource_class_kwargs={'source' : self.source, 'auth' : self.systemAuth})
        self.api.add_resource(GenerateTokenAPI, '/captcha22/api/v1.0/generate_token', endpoint='generate_token', resource_class_kwargs={'source': self.source, 'auth' : self.systemAuth})
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog="captcha22_api_server", description=("CAPTCHA22 API Server is the main interface to the CAPTCHA22 Server."))

    parser_group_hosting = parser.add_argument_group(title="Hosting Arguments")
    ...
    parser_group_datastore = parser.add_argument_group(title="Datastore Arguments")
